...
(
    SELECT 
        COUNT_BIG(DISTINCT cdmTable.VISIT_DETAIL_SOURCE_VALUE) + COUNT(DISTINCT CASE WHEN cdmTable.VISIT_DETAIL_SOURCE_VALUE IS NULL THEN 1 END) AS num_rows
    FROM dbo.VISIT_DETAIL cdmTable
...
(
    SELECT 
        COUNT_BIG(DISTINCT cdmTable.ADMITTING_SOURCE_VALUE) + COUNT(DISTINCT CASE WHEN cdmTable.ADMITTING_SOURCE_VALUE IS NULL THEN 1 END) AS num_rows
    FROM dbo.VISIT_DETAIL cdmTable
...
(
    SELECT 
        COUNT_BIG(DISTINCT cdmTable.DISCHARGE_TO_SOURCE_VALUE) + COUNT(DISTINCT CASE WHEN cdmTable.DISCHARGE_TO_SOURCE_VALUE IS NULL THEN 1 END) AS num_rows
    FROM dbo.VISIT_DETAIL cdmTable
